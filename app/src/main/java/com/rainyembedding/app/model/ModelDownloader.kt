...
class ModelDownloader(private val context: Context) {
    ...
    companion object {
        ...
        private const val MIN_FREE_SPACE_HEADROOM = 500_000_000L // 500MB 安全余量
    }
     ...
     * @param hfToken HuggingFace API Token（已于 HF 上 Accept Gemma License 后获取），传 null 则不鉴权
     * @return 下载任务 ID
     */
    fun startDownload(
        modelInfo: ModelInfo,
        destination: File,
        downloadUrl: String = modelInfo.url,
        hfToken: String? = null
    ): Long {
        ...
        val request = DownloadManager.Request(Uri.parse(downloadUrl)).apply {
            ...
            setDescription("${modelInfo.sizeMb} · RainyEmbedding")
            setDestinationInExternalFilesDir(context, null, subPath)
            setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED)
            setAllowedNetworkTypes(DownloadManager.Request.NETWORK_WIFI or DownloadManager.Request.NETWORK_MOBILE)
            setAllowedOverMetered(true)
            setAllowedOverRoaming(false)
            // HuggingFace 鉴权（需先在 HF 上 Accept Gemma License 并获取 token）
